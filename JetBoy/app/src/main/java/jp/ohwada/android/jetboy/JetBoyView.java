...
import jp.ohwada.android.jetboy.Explosion;
...
public class JetBoyView extends SurfaceView implements SurfaceHolder.Callback {
	...
	private final static String TAG = "JetBoy";
    	private final static String TAG_SUB = "JetBoyView";

    // disable, because too many logs
    private final static boolean DEBUG = false; 


// substitute by BACK key, since there is no DPAD key in device
    private final static int KEYCODE = KeyEvent.KEYCODE_BACK;

    // the number of asteroids that must be destroyed
    public static final int SUCCESS_THRESHOLD = 20;
    ...
    class JetBoyThread extends Thread implements OnJetEventListener {
        ...
        private void initializeJetPlayer() {
            ...
            log_d( "opening jet file DONE");

            mCurrentBed = 0;
            byte sSegmentID = 0;

            log_d( " start queuing jet file");
            ...
            log_d( " start queuing jet file DONE");
 ...
 */
        ...
        protected Object processKeyEvent(KeyGameEvent event, Object context) {
             log_d( "key code is " + event.keyCode + " " + (event.up ?
             "up":"down"));
            ...
            // presses
                ...
                if (event.keyCode == KEYCODE && (context == null)) {
                    return event;
                }
            }
...
// android_media_JetPlayer_setMuteFlag(): failed to update mute flag for track 23 with EAS error code -37
        ...
        protected void processJetEvent(JetPlayer player, short segment, byte track, byte channel,
            ...
            log_d( "onJetEvent(): seg=" + segment + " track=" + track + " chan=" + channel
        ...
        private void doCountDown() {
            log_d("Time left is " + mTimerLimit);
        ...
        public void onJetNumQueuedSegmentUpdate(JetPlayer player, int nbSegments) {
            log_d("onJetNumQueuedUpdate(): nbSegs =" + nbSegments);
        ...
        public void onJetPauseUpdate(JetPlayer player, int paused) {
            log_d("onJetPauseUpdate(): paused =" + paused);

        }

        // JET info: JET event listener interface implementation:
        public void onJetUserIdUpdate(JetPlayer player, int userId, int repeatCount) {
            log_d("onJetUserIdUpdate(): userId =" + userId + " repeatCount=" + repeatCount);
                    ...
                    if (m.getData().getString("STATE_LOSE") != null) {
                        ...
                        if (mHitTotal >= SUCCESS_THRESHOLD) {
                            ...
                            mTextView.setText("Sorry, You Lose! You got " + mHitTotal
                                    + ". You need " + SUCCESS_THRESHOLD + " to win.");
                        }
