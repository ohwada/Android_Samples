 ...
 * gerLargestSize
 */
protected Size getLargestSize( StreamConfigurationMap  map ) {
        prev_log(" getLargestSize");
    // For still image captures, we use the largest available size.
...
/**
  * getPreviewSize
  */
protected Size getPreviewSize(StreamConfigurationMap  map, int width, int height, int sensorOrientation) {
        prev_log("getPreviewSize");
        Size largest = getLargestSize( map );
 ...
 * fitTextureView
 */
protected void fitTextureView(Size previewSize) {
        prev_log("fitTextureView");
                // We fit the aspect ratio of TextureView to the size of preview we picked.
