...
import net.majorkernelpanic.streaming.video.HwCodecManager;
import net.majorkernelpanic.streaming.video.HwCodecManager.Codec;
 ...
 * </ul>
 * 
 * original : net.majorkernelpanic.streaming.hw.EncoderDebugger
 */
@SuppressLint("NewApi")
public class HwEncoderDebugger {

	public final static String TAG = "HwEncoderDebugger";
	...
	private final static int BITRATE = 1000000; // 1 Mbps

	/** Frame rate that will be used to test the encoder. */
	private final static int FRAMERATE = 20; // 20 fps
 ...
 * debug
 */	
	public synchronized static HwEncoderDebugger debug(Context context, int width, int height) {
		SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
 ...
 * debug
 */
	public synchronized static HwEncoderDebugger debug(SharedPreferences prefs, int width, int height) {
		HwEncoderDebugger debugger = new HwEncoderDebugger(prefs, width, height);
		debugger.debug();
		return debugger;
	}
 ...
 * 2019-08-01 added
 */
	public synchronized  void debug(MediaCodec mediaCodec) {
		if (VERBOSE) Log.d(TAG, "debug mediaCodec");
...
// colorFormat= 19 : COLOR_FormatYUV420Planar
		...
		if (VERBOSE) Log.d(TAG, "colorFormat= " + colorFormat);
 ...
 * constractor
 */
	public HwEncoderDebugger(SharedPreferences prefs, int width, int height) {
		mPreferences = prefs;
		mWidth = width;
		mHeight = height;
		mSize = width*height;
 ...
 */
	private void debug() {
		...
		Codec[] encoders = HwCodecManager.findEncodersForMimeType(MIME_TYPE);
		Codec[] decoders = HwCodecManager.findDecodersForMimeType(MIME_TYPE);
