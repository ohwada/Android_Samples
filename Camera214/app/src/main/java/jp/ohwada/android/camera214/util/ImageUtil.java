 ...
 * getOutputFile
 */ 
public File getOutputFile(Date date) {
    return FileUtil.getOutputFileInExternalStoragePublicDCIM(FILE_PREFIX, FILE_EXT_JPG, date);
}
...
public boolean saveImageAsJpeg(Image image, File file) {
        Bitmap original_bitmap = createBitmap(image );
        Bitmap resized_bitmap = resizeBitmap(original_bitmap, THUMBNAIL_SIZE);
        return saveBitmapAsJpeg(resized_bitmap, file);
}
